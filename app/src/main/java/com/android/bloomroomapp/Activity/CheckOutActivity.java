...
import static com.android.bloomroomapp.Model.Utils.TAG_medicine_list;
...
import com.android.bloomroomapp.Adapter.CartCustomAdapter;
import com.android.bloomroomapp.Model.Order;
...
public class CheckOutActivity extends AppCompatActivity {
    ...
    private void updateSellReport(double totalPrice) {
        ...
        for (int i = 0; i < productArrayList.size(); i++) {
            ...
            if (category.equals("Sympathyflowers")) {
                Sympathyflowers = Sympathyflowers + 1;
            } else if (category.equals("Housewarmingflowers")) {
                Housewarmingflowers = Housewarmingflowers + 1;
            } else if (category.equals("Birthdayflowers")) {
                Birthdayflowers = Birthdayflowers + 1;
            } else if (category.equals("Anniversary")) {
                Anniversary = Anniversary + 1;
            }else if (category.equals("NewBaby")) {
                NewBaby = NewBaby + 1;
            }else if (category.equals("ValentinesDay")) {
                ValentinesDay = ValentinesDay + 1;
            }else if (category.equals("Graduation")) {
                Graduation = Graduation + 1;
            }else if (category.equals("MothersDay")) {
                MothersDay = MothersDay + 1;
            }
        }

        Report report = new Report(Sympathyflowers, Housewarmingflowers, Birthdayflowers, Anniversary, NewBaby, ValentinesDay, Graduation, MothersDay, totalPrice);
    ...
    private void initAll() {
        ...
        radioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                ...
                if (movement.equals("Delivery")) {
                    ...
                    totalPayablePrice.setText("RS " + new DecimalFormat("##.##").format(order.getTotalPrice() + 10));
                    addressCardView.setVisibility(View.VISIBLE);
                } else {
                    deliveryChargesLayout.setVisibility(View.GONE);
                    totalPayablePrice.setText("RS " + new DecimalFormat("##.##").format(order.getTotalPrice()));
        ...
        orderPrice.setText("RS " + new DecimalFormat("##.##").format(order.getTotalPrice()));
